...
from API.app.cloud_services.aws_services.cognito import CognitoClient
...
import json
import logging
from ..cloud_services.aws_services.cognito import CognitoClient
...
def register(user: UserCreate):
    try:
        ...
        if user.phone_number:
            attributes.append({
                "Name": "phone_number",
                "Value": utils.normalize_phone(user.phone_number)  # must be +91XXXXXXXXXX
            })
        # Region / Country
        if user.region:
            attributes.append({
    ...
    if refresh_token:
        try:
            cognito.revoke_token(
                Token=refresh_token,
                ClientId=CLIENT_ID,
                ClientSecret=utils.get_secret_hash(
                    "dhusnic",
                    CLIENT_ID,
                    COGNITO_CLIENT_SECRET
                )

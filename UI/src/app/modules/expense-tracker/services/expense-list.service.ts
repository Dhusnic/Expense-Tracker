...
export class ExpenseListService {
    private readonly API_BASE_URL = environment.apiBaseUrl + '/transactions';
     ...
     */
    // In expense-list.service.ts
    getTransactions(
        filters: Partial<FilterOptions>,
        sort: SortOptions,
        pagination: PaginationConfig
    ): Observable<any> {  // Change from ExpenseListResponse to any
        const params = this.buildQueryParams(filters, sort, pagination);

        return this.http.get<any>(
            `${this.API_BASE_URL}/get_list`,
            { params }
        );
    }
